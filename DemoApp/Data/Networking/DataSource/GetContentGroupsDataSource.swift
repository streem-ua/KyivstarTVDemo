...
protocol GetContentGroupsDataSource {
    func getContentGroups() async throws -> [ContentGroupsResponse]
}

struct GetContentGroupsAPIImplamentation: GetContentGroupsDataSource {
    
    func getContentGroups() async throws -> [ContentGroupsResponse] {
        guard let url = URL(string: "\(APIEndpoint.getContentGroups.url)") else {
        ...
        do {
            let result = try JSONDecoder().decode([ContentGroupsResponse].self, from: data)
            return result
        } catch let error {
